...
class Edge(AbstractConcreteBase, ORMBase, DeclareLastEdgeMixin):
    ...
    def src_id(cls):
        ...
        if not src_table and hasattr(cls, "__src_class__"):
            src_table = NODE_TABLENAME_SCHEME.format(
    ...
    def dst_id(cls):
        ...
        if not dst_table and hasattr(cls, "__dst_class__"):
            dst_table = NODE_TABLENAME_SCHEME.format(
    ...
    def _get_subclasses_labeled(cls, label):
        return [c for c in cls.__subclasses__() if c.get_label() == label]

    @classmethod
    def _get_edges_with_src(cls, src_class_name):
        return [c for c in cls.__subclasses__() if c.__src_class__ == src_class_name]

    @classmethod
    def _get_edges_with_dst(cls, dst_class_name):
        return [c for c in cls.__subclasses__() if c.__dst_class__ == dst_class_name]
